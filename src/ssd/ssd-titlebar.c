...
ssd_titlebar_create(struct ssd *ssd)
{
	...
	FOR_EACH_STATE(ssd, subtree) {
		...
		add_scene_rect(&subtree->parts, LAB_SSD_PART_TITLEBAR, parent,
			width - theme->button_width * SSD_BUTTON_COUNT, theme->title_height,
			theme->button_width, 0, color);
		/* Buttons */
		...
		add_scene_button(&subtree->parts, LAB_SSD_BUTTON_ICONIFY, parent,
			...
			width - theme->button_width * 3, view);
		...
		struct ssd_part *btn_max_root = add_scene_button(
			...
			width - theme->button_width * 2, view);
		...
		add_scene_button_corner(&subtree->parts,
			...
			width - theme->button_width * 1, view);
	} FOR_EACH_END
			...
			case LAB_SSD_BUTTON_ICONIFY:
				if (is_direct_child(part->node, subtree)) {
					wlr_scene_node_set_position(part->node,
						width - theme->button_width * 3, 0);
				}
				continue;
			case LAB_SSD_BUTTON_MAXIMIZE:
				if (is_direct_child(part->node, subtree)) {
					wlr_scene_node_set_position(part->node,
						width - theme->button_width * 2, 0);
				}
				continue;
			case LAB_SSD_PART_CORNER_TOP_RIGHT:
				if (is_direct_child(part->node, subtree)) {
					wlr_scene_node_set_position(part->node,
						width - theme->button_width * 1, 0);
				}
				continue;
			default:
				continue;
...
ssd_update_title_positions(struct ssd *ssd)
{
	...
	struct theme *theme = view->server->theme;
	int width = view->current.width;
	int title_bg_width = width - theme->button_width * SSD_BUTTON_COUNT;
	...
	FOR_EACH_STATE(ssd, subtree) {
		...
		if (theme->window_label_text_justify == LAB_JUSTIFY_CENTER) {
			if (buffer_width + theme->button_width * 2 <= title_bg_width) {
...
ssd_update_title(struct ssd *ssd)
{
	...
	int title_bg_width = view->current.width
		- theme->button_width * SSD_BUTTON_COUNT;
